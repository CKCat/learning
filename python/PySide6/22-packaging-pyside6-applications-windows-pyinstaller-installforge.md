...
在这里，我们向 `app` 实例添加了 `.setWindowIcon` 调用。这定义了一个用于我们应用程序所有窗口的*默认*图标。如果你愿意，你也可以通过在窗口本身上调用 `.setWindowIcon` 来为每个窗口单独设置图标。
...
如果你运行构建，你应该会看到你的 `.ico` 文件现在在输出文件夹 `dist` 中，准备好与你的应用程序一起分发。
